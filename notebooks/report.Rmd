...
model.polynomial.forecast <- predict(model.polynomial, data.future, interval = "confidence", level = 0.95)
...
# Create forecasts
model.linear.forecast <- predict(model.linear, data.future, interval = "confidence", level = 0.99)
model.linear.predicted <- cbind(data.future, model.polynomial.forecast) %>%
...
ggplot() +
  geom_line(aes(x = index, y = average), color = 'black', data = co2_present) +
...
model.arima.predicted <- data.frame(index = yearmonth(rownames(model.arima.predicted)), average = model.arima.predicted$Point.Forecast)
