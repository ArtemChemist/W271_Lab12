...
\begin{figure}[h]
  ...
  \begin{subfigure}[t]{0.6\textwidth}
    \centering\includegraphics[width=\textwidth]{./figures/datamodels.png}
    \caption{Historic trend and the models.\label{fig:carbon}}
  \end{subfigure}
  \hfill
  \begin{subfigure}[t]{0.35\textwidth}
    \includegraphics[width=\textwidth]{./figures/plot_decomposition.png}
...
\begin{figure}[h]
  ...
  \begin{subfigure}[t]{0.45\textwidth}
    \centering\includegraphics[width=\textwidth]{./figures/plot_lin_mod_resid.png}
    \caption{Linear model\label{fig:lin_model_resid}}
  \end{subfigure}
  \hfill
  \begin{subfigure}[t]{0.45\textwidth}
    \centering\includegraphics[width=\textwidth]{./figures/plot_quad_mod_resid.png}
...
ggsave(filename = './figures/datamodels.png', plot = quad_plot, device = "png", units = c("in"), height= 3, width=6)
```
...
# Make df with the future dates in the date format and stitch it with predictions
polynomdate <- as.Date(seq(ISOdate(1998,01,01), by = "month", length.out = frcst_length))
model.wmonth.predicted <- cbind.data.frame(polynomdate , model.wmonth.forecast)
```
...
\begin{figure}[h]
  ...
  \begin{subfigure}[t]{0.33\textwidth}
    \centering\includegraphics[width=\textwidth]{./figures/best_arima_resid.png}
...
png(file = './figures/best_arima_resid.png', height=6, width=5, units = c("in"), res = 600)
...
forecast_2models_plot <- ggplot() + 
  ...
  geom_line(aes(x = polynomdate, y = fit, color = "Quadratic model w month"), size = 0.25, data = model.wmonth.predicted) +
  labs(x = '', y = TeX(r'(\[$CO_2$\], ppm)')) + 
...
forecast_plot <- ggplot() + 
  geom_line(aes(x = my_date, y = value), color = 'black', data = co2_df) +
  geom_line(aes(x = my_date, y = mean), color = "red", data = df_frcst) + 
  geom_ribbon(aes(x = my_date, ymin = lower, ymax = upper), fill="yellow", alpha=0.35, data = df_frcst)+
  geom_ribbon(aes(x = polynomdate, ymin = lwr, ymax = upr), fill = "green", alpha = 0.65, data = model.wmonth.predicted) +
  geom_line(aes(x = polynomdate, y = fit), color = 'darkgreen', data = model.wmonth.predicted) +
  geom_hline(yintercept = 420, linetype='dotted', col = 'blue', size = 1) +  
  geom_hline(yintercept = 500, linetype='dotted', col = 'blue', size = 1)
  labs(
    title = TeX(r'(Monthly Mean $CO_2$ with Forecasted Values)'),
    subtitle = 'Forecasted levels (with 95% CI) in yellow',
    x = '',
    y = TeX(r'($CO_2$ parts per million)')
  ) + xlim (as.Date("1990-01-01"), as.Date("2100-01-01")) + ylim(350, 700)

ggsave(filename = "./figures/plot1_frcst2022.png", plot = forecast_plot, device = "png", units = c("in"), height = 4, width = 10)
```


\begin{figure}[ht]
  \includegraphics[width=.95\linewidth]{./figures/plot1_frcst2022.png}
  \caption{Forecast for future CO2 concentration \label{fig:frcst}}
  \begin{figurenotes}
    Unrealistically narrow prediction interval for polynomial model
  \end{figurenotes}
\end{figure}
...
| PPM    | For the first time | Expected         | Last time         |
|--------|--------------------|------------------|-------------------|
...
|-------------------------------------|----------------------------------------|-------------------------------------|
| `r round(prediction.2100.lower, 1)` | `r round(prediction.2100.expected, 1)` | `r round(prediction.2100.upper, 1)` |
# Conclusions 
