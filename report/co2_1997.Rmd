...
Atmospheric $CO_{2}$ level is plotted in \autoref{fig:carbon}(black trace), and shows a worrisome increasing trend with strong seasonal component. \hfill\break\indent \autoref{fig:notrend_plots} shows diagnostic information for this timeseries. After removing yearly variability by differencing with lag of 12 months, the series remains non-stationary, with visible upward trend (not shown). Further detrending with lag 1 brings it close to stationary (\autoref{fig:notrend_ts}). ACF plot (\autoref{fig:notrend_acf}) of this de-trended series shows significant peaks only at lags 1 to 3 and few peaks around lag 12, without slow-decaying pattern. This indicates an MA(3) process and the need for seasonal component in SARIMA model. Similarly, PACF plot (\autoref{fig:notrend_pacf}) has a few significant peaks up to lag 3 and a cluster of significant peaks at lag 12. These features are indicative of AR(3) process, and also point tot eh need of seasonal component in SARIMA model. A grid search of model parameters, including differencing parameter will be required for precise model allocation. 
