...
```{r Split into test and train, warning=FALSE}
my_lag <- 12
...
best_bic <- find_bic(hist_arima, co2_ts$value)
```
...
df_toPlot <- cbind.data.frame(polynomdate, frcst_2022$mean, frcst_2022$lower, frcst_2022$upper)
df_toPlot$week <- yearweek(df_toPlot$polynomdate)

df_toPlot <- df_toPlot[c(1,2,4,6)]
names(df_toPlot) <- c("my_date", "mean", "lower", "upper")

forecast_plot <- ggplot() + 
  geom_line(aes(x = my_date, y = value), color = 'black', data = co2_df) +
  geom_line(aes(x = my_date, y = mean), color = "red", data = df_toPlot) + 
  geom_ribbon(aes(x = my_date, ymin = lower, ymax = upper), fill="yellow", alpha=0.35, data = df_toPlot)+
  ...
  ) + xlim (as.Date("1990-01-01"), as.Date("2122-01-01")) + ylim(350, 700)
...
dates <- df_toPlot$my_date

# When CO2 is expected to hit 420ppm
first_time.420 <- first.date(df_toPlot$upper, 420)
expected.420 <- first.date(df_toPlot$mean, 420)
last_time.420 <- first.date(df_toPlot$lower, 420)

# When CO2 is expected to hit 500ppm
first_time.500 <- first.date(df_toPlot$upper, 500)
expected.500 <- first.date(df_toPlot$mean, 500)
last_time.500 <- first.date(df_toPlot$lower, 500)

# Expected CO2 levels in the year 2100
prediction.2100.index <- match("2100-01-01", dates)
prediction.2100.lower <- df_toPlot$lower[prediction.2100.index]
prediction.2100.expected <- df_toPlot$mean[prediction.2100.index]
prediction.2100.upper <- df_toPlot$upper[prediction.2100.index]
```
