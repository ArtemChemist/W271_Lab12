...
pred_plot <- ggplot() +
  ...
  geom_ribbon(aes(x = polynomdate, ymin = lwr, ymax = upr), fill = "yellow", alpha = 0.65, data = model.wmonth.predicted) +
  geom_line(aes(x = polynomdate, y = fit), color = 'red', data = model.wmonth.predicted) +
  labs(
...
\begin{figure}[ht]
  ...
  \begin{figurenotes}
    Unrealistically narrow prediction interval 
  \end{figurenotes}
\end{figure}
