...
# Calculate number of weeks passed for each date
start_date <- ymd("1974-05-17")
df$weeks <- floor(difftime(df$date, start_date, units = "weeks"))
# Aggregate by weeks
df <- df %>%
  group_by(weeks) %>%
  summarise(date = min(date), mean_ppm = mean(ppm))

# df$first_day <- floor_date(df$date, unit = "week", week_start = 1)
