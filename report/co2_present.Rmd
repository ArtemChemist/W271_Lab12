...
# Create date column 
df$date <- make_datetime(year = df$year,
                                    month= df$month,
                                    day = df$day)
...
# Aggregate by weeks:
  #Find the date for the first day of that week
  df$week <- floor_date(df$date, unit = "week", week_start = 1)
  # Aggregate by weeks
  df <- df %>% group_by(week) %>% summarise(ppm = mean(ppm))
  
# Convert to tsibble  
co2_present <-df %>% mutate(week = as.Date(week)) %>% 
  as_tsibble(key = "ppm", index = "week")
```
...
\begin{figure}
  ...
  \begin{figurenotes}
    $CO_2$ concentration from 1974 onward continues to exhibit ever accelerating growth, with strong seasonal oscillations. This trends points to the need of de-seasoning before further analysis.
  \end{figurenotes}
\end{figure} 
...
my_lag <- 52
co2_present <- mutate(co2_present, diff = difference(ppm, lag=my_lag))
co2_diff <- slice(co2_present, my_lag+1:nrow(co2_present))
```
...
diff_ts <- co2_diff %>% 
  ggplot() + 
  aes(x = time_index, y = diff) + geom_line()

acf_plot<-co2_diff %>%
  ACF(diff, type="correlation") %>% 
  autoplot()  + labs (x = "lag", y = "ACF")

pacf_plot<-co2_diff %>%  ACF(diff, type="partial") %>% 
  autoplot() + labs (x = "lag", y = "PACF")

grid.arrange(diff_ts, acf_plot, pacf_plot, nrow=1)
```
