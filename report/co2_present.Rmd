...
\begin{figure}[h]
  ...
  \begin{subfigure}[b]{0.45\textwidth}
    ...
    \caption{Models on raw data \label{fig:frcst2y}}
  \end{subfigure}
  \hfill
  \begin{subfigure}[b]{0.45\textwidth}
    \includegraphics[width=\textwidth]{./figures/plot2_frcst_nsa.png}
    \caption{Models on de-seasoned data \label{fig:frcst2y_raw}}
  \end{subfigure}
...
frcst_arima_nsa <- ggplot() +
  ...
  labs(x = element_blank(), y = TeX(r'(\[$CO_2$\], ppm)')) + ylim(-1, 5) + 
  xlim (ISOdate(2021, 06, 01), ISOdate(2023, 06, 01)) +
   # Legend colors
  scale_color_manual(name = "", values = c("Observed data" = "black", 
                                         "ARIMA(0.1.2)" = "orange",
...
ggsave(filename = "./figures/plot2_frcst_nsa.png", plot = frcst_arima_nsa, device = "png", units = c("in"), height = 4, width = 10)
```
